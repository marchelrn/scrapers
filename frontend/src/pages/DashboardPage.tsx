...
import { configsApi } from '../api/configs' // Pastikan API untuk config di-import
...
export function DashboardPage() {
  ...
  const fetchData = async () => {
    try {
      const [sumRes, jobsRes, configRes] = await Promise.all([
            ...
            <div className="table-wrap">
              <table className="table">
                ...
                {recentJobs.length === 0 ? (
                    ...
                    recentJobs.map((j) => {
                      // 2. Gunakan .find() untuk mencocokkan config dengan j.config_id
                      const config = scrapingConfig.find((s) => s.id === j.config_id);
